...
/*=======================================================[ WELCOME ]==============================================================*/                  	    
		...
		try {
			...
			if (anu.action == 'add') {
				...
				usrw = nayla.contacts[num] != undefined ? nayla.contacts[num].vname || nayla.contacts[num].notify : undefined
				let buff = await getBuffer(ppimg) 
				const imagew = await new Canvas.Welcome()
  .setUsername(usrw)
  ...
  .setBackground("https://wallpaperaccess.com/full/3053381.jpg")
  .toAttachment();
  const base64 = `data:image/jpeg;base64,${imagew.toBuffer().toString('base64')}`
  await sleep(2000)				
				await nayla.sendMessage(mdata.id, imagew.toBuffer(), MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				usrw = nayla.contacts[num] != undefined ? nayla.contacts[num].vname || nayla.contacts[num].notify : undefined
				let buff = await getBuffer(ppimg) 
				const imagew = await new Canvas.Goodbye()
  .setUsername(usrw)
  .setDiscriminator("0000")
  .setMemberCount(mdata.participants.length)
  .setGuildName(mdata.subject)
...
/*=====================================================[ API VIDEFIKRI ]==============================================================*/                  	                 
            ...
            */const vnayla = 'BEGIN:VCARD\n' 
			...
			if (!isGroup && isCmd) console.log('\x1b[1;31m=\x1b[1;37m>', '[\x1b[1;32mBOTZ5\x1b[1;37m]', time, color(command), 'dari', color(sender.split('@')[0]), 'args :', color(args.length))
			if (!isGroup && !isCmd) console.log('\x1b[1;31m=\x1b[1;37m>', '[\x1b[1;31mBOTZ5\x1b[1;37m]', time, color('Pesan'), 'dari', color(pushname), 'args :', color(args.length))
			if (isCmd && isGroup) console.log('\x1b[1;31m=\x1b[1;37m>', '[\x1b[1;32mBOTZ5\x1b[1;37m]', time, color(command), 'dari', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			if (!isCmd && isGroup) console.log('\x1b[1;31m=\x1b[1;37m>', '[\x1b[1;31mBOTZ5\x1b[1;37m]', time, color('Pesan'), 'dari', color(pushname), 'in', color(groupName), 'args :', color(args.length))
            const xxx = '```' 
//============================
// AUTHOR FELIXCRACK
//============================ 
menuZ = `╭──❲ ${xxx}WHATSAPP${xxx} ❳
│${tz} ${xxx} DUEÑO : ${ownername}${xxx}
│${tz} ${xxx} AUTOR : FELIXCRACK 409
│${tz} ${xxx}MES : ${bulan}${xxx}
│${tz} ${xxx}CHAT : ${tchat}${xxx}
╰──❲ 2011 ❳
╭──❲ ${xxx}📜 REGLAS 📜${xxx} ❳ 
│${tz} ${xxx}NO SPAM AL BOT${xxx}
│${tz} ${xxx}NO AÑADIR AL BOT A GRUPOS${xxx}
│${tz} ${xxx}NO SATURARLO${xxx}
│${tz} ${xxx}NO LLAMADAS AL BOT${xxx}
╰──❲ ${xxx}2021${xxx} ❳
╭──❲ ${xxx}LISTMENU${xxx} ❳
...
│${tz} ${xxx}${prefix}islammenu${xxx}
│${tz} ${xxx}${prefix}sertifikat${xxx}
│${tz} ${xxx}${prefix}editfoto${xxx}
╰──❲ ${xxx}2021${xxx} ❳
╭──❲ ${xxx}THX TO${xxx} ❳
│${tz} ${xxx}🌚${xxx}
│${tz} ${xxx} FELIXCRACK${xxx}
...
const grupmenu = `╭──❲ ${xxx}WHATSAPP${xxx} ❳  
...
const makermenu = `╭──❲ ${xxx}WHATSAPP${xxx} ❳
...
const downloadmenu = `╭──❲ ${xxx}WHATSAPP${xxx} ❳
...
│${tz} ${xxx}${prefix}ayatkursi${xxx}
│${tz} ${xxx}${prefix}bacaansholat${xxx}
│${tz} ${xxx}${prefix}doaharian${xxx}
│${tz} ${xxx}${prefix}kisahnabi${xxx}
│${tz} ${xxx}${prefix}niatsholat${xxx}
│${tz} ${xxx}${prefix}tahlil${xxx}
│${tz} ${xxx}${prefix}wirid${xxx}
╰──❲ ${xxx}BY ${ownername}${xxx} ❳`
...
case 'attp':
				...
				attp2 = await getBuffer(`https://xteam.xyz/ttp?file&text=${body.slice(6)}`)
...
case 'semoji':
                    ...
                    ini_nayla = await getBuffer(`https://videfikri.com/api/emojitopng/?emojicode=${emoji}`)
...
/*====================================================[ CASE LOLTEAM ]==============================================================*/                    	 
                ...
                reply(naylachan)
                dapuhy = await getBuffer(`https://api.lolhuman.xyz/api/${command}?apikey=${LolHuman}&text=${dppa}`)
                nayla.sendMessage(from, dapuhy, sticker, {quoted: nay1})
...
/*====================================================[ CASE BY NAYLA ]==============================================================*/                    	 
					...
					reply(`Enlace detectado😤 ${sender.split("@")[0]} serás expulsado del grupo en 5 Segundo`)
					setTimeout( () => {
                    ...
                    const F3 = fs.readFileSync('sound/criss.mp3')
                    nayla.sendMessage(from, F3, audio, {quoted: nay1, ptt : true})
                    }
                    if (budy.includes(`Pato`)){
                    const F3 = fs.readFileSync('src/pato.webp')
                    nayla.sendMessage(from, F3, sticker, {quoted: nay1})
                    }
